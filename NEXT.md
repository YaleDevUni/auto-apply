...
- [x] ~~잠재 그늘 두 곳 — 지역변수가 같은 이름의 모듈 함수를 가린다~~
      (2026-08-18 회차에서 고침) — `assemble.py:896`의 지역변수 `review_notes`를
      `fix_notes`로, `workflows/prepare_application.py:68`의 재대입
      `log = logging.getLogger(__name__)`를 제거해 모듈 로거를 그대로 쓰게 했다.
      둘 다 가려진 이름을 그 함수 안에서 호출하지 않아 버그는 아니었지만
      (`workflows/context.py:87`은 호출해서 실제로 죽었던 사례 — 이미 고침),
      같은 클래스라 재발을 막았다. 검증: py_compile 통과, pytest 163건 통과.
