...
2. [x] **텔레그램 `notify()` 길이 제한** (2026-08-18 새벽 회차에서 고침) —
       `notify_with_buttons`(`text[:4096]`)·`send_photo`(`caption[:1024]`)는
       이미 잘랐는데 `notify()`만 빠져 있었다. 긴 텍스트를 보내면 텔레그램이
       400을 돌려주고 광범위 except가 삼켜서 그 알림이 흔적 없이 사라지는
       문제였다 — `notify()`에도 `text[:4096]` 적용.
