...
- [ ] `claude setup-token`(1년 OAuth)을 launchd plist의
      ...
      현재 plist 4개 전부 0건
- [x] ~~프롬프트 캐시 프리픽스 맞추기~~ (2026-08-18 회차에서 고침) — 조립
      (`_editor_prompt`)과 검수(`review_editor`)가 같은 가이드·채용공고를 쓰는데
      **지시문을 앞에 두고 가이드를 뒤에 붙이는 순서가 서로 달라** 첫 바이트부터
      어긋났다. 캐시는 위치 0부터의 프리픽스 일치로 결정되므로, 가이드가 글자
      그대로 같아도 그 앞에 오는 지시문이 다르면 캐시가 안 맞는다. 두 프롬프트
      모두 `# 작성 가이드` → `# 채용공고` 블록을 맨 앞으로 옮기고 함수별 지시문을
      그 뒤(`---` 구분선 다음)로 밀었다 — 지시문 문구는 그대로 두고 위치만
      옮겼다(`_editor_prompt`의 "아래 [작성 가이드]…"만 "위 [작성 가이드]…"로
      고침, 가이드가 이제 위에 있으므로). `_review_and_fix`가 조립 → 검수 →
      (필요시) 재작성 → 재검수를 같은 공고에서 이어 부르므로 네 호출 모두 이
      프리픽스를 공유한다.
      검증: 더미 job/guide로 두 프롬프트를 실제로 만들어 공통 프리픽스 길이를
      계산 — `# 작성 가이드`+`# 채용공고`+구분선 블록 전체(1132자)가 정확히
      공통 프리픽스이고 그 다음 바이트부터 지시문이 갈라짐을 확인(수정 전에는
      이 블록이 서로 다른 위치에 있어 공통 프리픽스가 사실상 0이었을 것).
      pytest 163건 통과(회귀 없음). 실제 캐시 적중률(cache_read_input_tokens)
      개선은 이번 회차에서 실물 조립/검수를 돌려보지 않아 미확인 — 다음 실행
      로그에서 확인할 것.
