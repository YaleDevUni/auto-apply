...
- [ ] 잠재 그늘 두 곳 — 지역변수가 같은 이름의 모듈 함수를 가린다.
      `assemble.py:840`(`review_notes`), `workflows/prepare_application.py:68`(`log`).
      둘 다 가려진 이름을 그 함수 안에서 **호출하지 않아** 지금은 버그가 아니다
      (`workflows/context.py:87`은 호출해서 실제로 죽었다 — 2026-08-18 회차에서
      `job_row`로 고침). 나중에 그 함수에 한 줄만 더해도 UnboundLocalError가 나므로
      건드릴 일이 생기면 그때 이름부터 바꿀 것
