...
flowchart TD
    ...
    subgraph repair["자가복구"]
        ...
        P1 --> P2{"위험도"}
        P2 -->|low| P3["수행 → 관문 검사<br/>(검증 명령도 실제 통과해야 함)"]
        P2 -->|medium·high| P6["폰 승인 대기<br/>새벽이면 09시에"]
        P6 -->|승인| P3
        P3 -->|통과| P4["main에 커밋<br/>+ 커밋 메시지 알림"]
        P3 -->|실패| P5["브랜치 강등<br/>재승인하면 P3로 재시도"]
    end
...
| `_ensure_summary_length()` | Sonnet 5 | 간단 소개가 550자 미만일 때만. 전체 재생성이 아니라 그 필드만 다시 묻는다 |
| `review_editor()` | Sonnet 5 | **실제 제출물을 보는 눈.** 사실 저장소와 대조해 산문 속 날조를 잡는다. 스킬 대조는 코드가 한다(`unknown_skills`). 날조가 있으면 재작성 1회, 그래도 남으면 승인 캡션에 ⚠️ — 차단하지 않는다 |
...
- [x] **강조 순서를 직무명 표가 아니라 공고 자격요건이 정한다** (2026-08-17) —
      §7이 `직무명 → 1·2·3순위` 고정표라, 백엔드 공고의 자격요건에 AWS·Docker가
      있어도 (D)(E)가 통째로 빠졌다. 이제 `자격요건 → §3 근거 매핑 → 필수요건을
      많이 덮는 순`이고, 옛 표는 §7-4 기본값으로 내렸다
- [x] **간단 소개에 대표 프로젝트 링크**(§8) — 링크 칸은 사본이 들고 오는 고정값이라
...
- [x] **실제 제출물 경로에 검수 연결** (2026-08-17) — `review()`가 마크다운 경로에만
      있어서, 지원에 나가는 `build_editor_json`에는 보는 눈이 없었다(실측:
      `to_editor_json` 36회 / `review` 2회). 스킬은 코드가 가이드 전문과 대조해
      지우고(오탐 0), 산문 날조는 Sonnet 5가 본다. **차단하지 않고** 승인 캡션에
      ⚠️로 띄운다 — 검수 모델이 4건에 1건씩 잘못된 날조를 달기 때문
...
- [x] **도는 동안 자동지원 보류** — 고장 난 채로 지원이 계속 나가지 않는다.
      ...
      안 풀린다
- [x] **승인되면 브랜치 대기 없이 바로 main에 반영** (2026-08-17) — 예전엔
      승인해도 브랜치에만 남아 사람이 `git merge`를 또 해야 했다(로컬 브랜치라
      PR도 아닌데 이중 관문이었다). 이제 승인 + 결정론 관문 통과만으로 main에
      간다. 관문은 승인 여부와 무관하게 무조건 건다. 자세한 내용은
      [NEXT.md](NEXT.md) 맨 위
- [x] 자가복구 **첫 실주행** (2026-08-17) — `error_queue`가 실제로 잡은 고장
      2건을 이 세션에서 사람이 직접 고치고 실측 검증 후 main에 커밋했다(자동
      fix-run은 안 거침 — 계획이 나온 뒤 코드를 이미 손으로 고쳐서 돌리면
      중복 작업이라). 자동 계획 → 자동 승인 → 자동 fix-run까지 이어지는
      전 구간을 사람 개입 없이 돌려본 것은 아직 아니다. 자세한 내용은
      [NEXT.md](NEXT.md) 맨 위
