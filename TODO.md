...
### P0 — 체인을 닫는 것
- [ ] **`autoapply --live` 실제 제출 검증**. dry-run은 6스텝을 통과하지만
      진짜 제출은 한 번도 이 경로로 안 해봤다. 이게 안 되면 나머지는 의미가 없다
- [ ] **사본 경로로 만든 이력서를 실제 지원에 써보기**. 지원 폼에서 제목으로
      고르는데, `#{n}` 붙은 제목이 실제로 잡히는지 확인 안 됐다

### P1 — 커버리지
- [ ] biz/presales 트랙 실주행. 지금까지 검증은 dev뿐이다
...
### P3 — 자가 개선
- [ ] 레시피 빌더에 비전 연결 (`vision.ask()`는 준비됨)
- [ ] 셀렉터가 깨졌을 때 화면을 보고 레시피를 고치는 경로
